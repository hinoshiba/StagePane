...
StagePane 0.1.0 has no third-party runtime code, SDK, font, model, media, or
binary dependency.
...
practices such as signed releases, notarization, documentation, and license
inventory. No Youyaku source code, brand asset, model, binary dependency, or
license notice is incorporated into StagePane.

XcodeGen 2.45.4 may optionally regenerate the checked-in Xcode project during
development. XcodeGen is MIT-licensed; neither its executable nor source is
bundled in StagePane, and it is not required to build the checked-in project.
Its license and non-runtime status are recorded in `docs/LICENSE_AUDIT.md`.
