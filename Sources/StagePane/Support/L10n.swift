...
enum L10n {
    ...
    static var sourceRemovalConfirmationMessage: String {
        text(
            "このソースの画面取得を終了し、Stageとプレビューからレイヤーと配置を削除します。この操作は取り消せません。",
            "This stops capturing the source and removes its layer and layout from the Stage and preview. This can’t be undone."
        )
    }

    static var detachedLayerRemovalConfirmationMessage: String {
        text(
            "このレイヤーに保持している配置と切り抜きをStageから削除します。この操作は取り消せません。共有元を選び直すだけなら、解除せず「選び直す」を使ってください。",
            "This removes the layer's retained placement and crop from the Stage. This can’t be undone. To select content for it again, cancel and use Select Again instead."
        )
    }

    static var stopAllAndRemoveLayersTitle: String {
        text(
            "すべて停止してレイヤーを削除…",
            "Stop All and Remove Layers…"
        )
    }

    static var stopAllAndRemoveLayersConfirmationTitle: String {
        text(
            "すべて停止して、すべてのレイヤーを削除しますか？",
            "Stop all capture and remove every layer?"
        )
    }

    static var stopAllAndRemoveLayersConfirmationMessage: String {
        text(
            "すべての画面取得を終了し、接続中と再接続待ちの全レイヤー、その配置と切り抜きを削除します。この操作は取り消せません。",
            "This ends all screen capture and removes every connected or disconnected layer, including its placement and crop. This can’t be undone."
        )
    }

    static var confirmStopAllAndRemoveLayersTitle: String {
        text(
            "すべて停止して削除",
            "Stop All and Remove Layers"
        )
    ...
    static var sourceRemovalAccessibilityHint: String {
        text(
            "画面取得を停止し、レイヤーと配置を削除する前に確認を表示します。",
            "Opens a confirmation before capture is stopped and the layer and layout are removed."
        )
    }
    ...
    static var sourceNeedsReselectionTitle: String {
        text("共有元を選び直してください", "Select Source Again")
    }

    static func reselectLayerTitle(_ sourceTitle: String) -> String {
        text(
            "「\(sourceTitle)」を選び直す",
            "Select Again for “\(sourceTitle)”"
        )
    }

    static func sourceNeedsReselectionHint(_ sourceTitle: String) -> String {
        text(
            "共有が終了した\(sourceTitle)レイヤーへ、Appleの共有ピッカーで新しい対象を設定します。配置と切り抜きは保持されています。",
            "Use Apple's sharing picker to select content again for the \(sourceTitle) layer. Its placement and crop are preserved."
        )
    }
    ...
    static var cropLayerModeTitle: String {
        text("レイヤーの切り抜き", "Layer Crop")
    }

    static func cropEditingLayerTitle(_ sourceTitle: String) -> String {
        text(
            "「\(sourceTitle)」を切り抜き中",
            "Cropping “\(sourceTitle)”"
        )
    }

    static func cropLayerActionHint(_ sourceTitle: String) -> String {
        text(
            "\(sourceTitle)レイヤーだけを全面表示し、Stageへ見せる範囲を下書きします。",
            "Show only the \(sourceTitle) layer at full size and draft its visible Stage area."
        )
    }

    static var perLayerCropEntryHint: String {
        text(
            "切り抜きは、各レイヤーの切り抜きボタンから開始します。",
            "Start cropping from the crop button on each layer."
        )
    }
    ...
    static func stageInteractionModeDetail(
        ...
        case .crop:
            text(
                "対象レイヤーの表示範囲を下書きします。Stageは適用まで変わりません。",
                "Draft the target layer's visible area. The Stage changes only when you apply it."
            )
        case .annotate:
